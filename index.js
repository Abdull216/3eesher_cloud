...
// ==================== 🗄️ DATABASE (ALL CONTENT & 30 LINKS RESTORED) ====================
function getData() {
...
function getDefaultData() {
    return {
        ...
        moneyLinks: [
            { name: 'Upwork', url: 'https://www.upwork.com', active: true, clicks: 0, icon: '💼' },
            { name: 'Fiverr', url: 'https://www.fiverr.com', active: true, clicks: 0, icon: '🎨' },
            { name: 'Freelancer', url: 'https://www.freelancer.com', active: true, clicks: 0, icon: '🖥️' },
            { name: 'ClickBank', url: 'https://www.clickbank.com', active: true, clicks: 0, icon: '💰' },
            { name: 'ShareASale', url: 'https://www.shareasale.com', active: true, clicks: 0, icon: '🤝' },
            { name: 'CJ Affiliate', url: 'https://www.cj.com', active: true, clicks: 0, icon: '🔗' },
            { name: 'Rakuten', url: 'https://www.rakuten.com', active: true, clicks: 0, icon: '🇯🇵' },
            { name: 'Amazon Associates', url: 'https://affiliate-program.amazon.com', active: true, clicks: 0, icon: '📦' },
            { name: 'eBay Partner', url: 'https://www.ebaypartnernetwork.com', active: true, clicks: 0, icon: '🏷️' },
            { name: 'Etsy Affiliate', url: 'https://www.etsy.com/affiliates', active: true, clicks: 0, icon: '🎁' },
            { name: 'Shopify Affiliate', url: 'https://www.shopify.com/affiliates', active: true, clicks: 0, icon: '🛒' },
            { name: 'Teachable', url: 'https://teachable.com', active: true, clicks: 0, icon: '📝' },
            { name: 'Udemy', url: 'https://www.udemy.com', active: true, clicks: 0, icon: '📚' },
            { name: 'Coursera', url: 'https://www.coursera.org', active: true, clicks: 0, icon: '🎓' },
            { name: 'Skillshare', url: 'https://www.skillshare.com', active: true, clicks: 0, icon: '✂️' },
            { name: 'YouTube', url: 'https://www.youtube.com/creators/', active: true, clicks: 0, icon: '🎬' },
            { name: 'TikTok', url: 'https://www.tiktok.com/creators/', active: true, clicks: 0, icon: '📱' },
            { name: 'Instagram', url: 'https://creators.instagram.com', active: true, clicks: 0, icon: '📸' },
            { name: 'Facebook', url: 'https://www.facebook.com/creators', active: true, clicks: 0, icon: '👥' },
            { name: 'Medium', url: 'https://medium.com/creators', active: true, clicks: 0, icon: '✍️' },
            { name: 'Substack', url: 'https://substack.com', active: true, clicks: 0, icon: '📧' },
            { name: 'Rev', url: 'https://www.rev.com/freelancers', active: true, clicks: 0, icon: '🎤' },
            { name: 'UserTesting', url: 'https://www.usertesting.com', active: true, clicks: 0, icon: '✅' },
            { name: 'Swagbucks', url: 'https://www.swagbucks.com', active: true, clicks: 0, icon: '🎁' },
            { name: 'Survey Junkie', url: 'https://www.surveyjunkie.com', active: true, clicks: 0, icon: '📊' },
            { name: 'Appen', url: 'https://appen.com', active: true, clicks: 0, icon: '🤖' },
            { name: 'Remotasks', url: 'https://www.remotasks.com', active: true, clicks: 0, icon: '⚙️' },
            { name: 'Amazon KDP', url: 'https://kdp.amazon.com', active: true, clicks: 0, icon: '📖' },
            { name: 'Redbubble', url: 'https://www.redbubble.com', active: true, clicks: 0, icon: '👕' },
            { name: 'Teespring', url: 'https://teespring.com', active: true, clicks: 0, icon: '🛍️' }
        ],
        storeLinks: [
        ...
        successStories: [
            { id: 1, name: 'Ahmed from Kano', after: '$2,500/month', story: 'Ahmed was a civil servant earning N80,000/month. He started with Fiverr and added ClickBank. Today he earns $2,500/month and owns a house.', avatar: '👨‍💼', color: '#10b981' },
            { id: 2, name: 'Fatima from Cairo', after: '$1,800/month', story: 'Engineering student who started with data entry on Upwork. Now she supports her family with US client income.', avatar: '👩‍🎓', color: '#f59e0b' },
            { id: 3, name: 'TICHER (Founder)', after: 'Built 3EESHER-CLOUD', story: 'Failed for 2 years before finding the digital wealth formula. Now dedicated to helping 10,000 others.', avatar: '🚀', color: '#fbbf24' }
        ],
        blogPosts: [],
        contracts: [ { title: "Terms of Engagement", content: "By using this platform, you agree to our digital resource protection agreement." } ],
        ...
        privacyContent: {
            ...
            details: 'We collect information you provide directly to us, such as email address and name when you register for the library or subscribe to our newsletter. We also automatically collect IP addresses and browser types.'
        }
    };
}
...
app.post('/admin/upload-logo', checkAdmin, upload.single('logo'), (req, res) => {
    ...
    res.send('<script>alert("Logo Card Updated!"); window.location.href="/super-admin";</script>');
});
...
app.post('/admin/create-blog', checkAdmin, upload.single('image'), (req, res) => {
    ...
    saveData(data); res.send('<script>alert("Manual Blog Saved!"); window.location.href="/super-admin";</script>');
});
...
app.post('/api/bot-command', checkAdmin, (req, res) => {
    const { cmd } = req.body;
    let reply = `Bot executed [${CMD_LIST[parseInt(cmd)-1]}] successfully. Action logged on disk.`;
    res.json({ reply });
});
...
app.get('/', (req, res) => {
    ...
    <style>
        ...
        .logo-card{max-width:950px; width:95%; border-radius:25px; border:4px solid var(--highlight); box-shadow:0 20px 60px rgba(0,0,0,0.7);}
...
<body>
    ...
    <header>
        ...
        <div style="max-width:800px; margin:0 auto; background:rgba(0,0,0,0.5); padding:40px; border-radius:20px; text-align:left;">
            <h3 style="color:var(--highlight);">Digital Education & Income Empire</h3>
...
// ==================== 💻 SUPER ADMIN DASHBOARD (FULL LABELED BOT) ====================
app.get('/super-admin', checkAdmin, (req, res) => {
    ...
    <div class="sidebar"><h2>CEO HUB</h2><a onclick="show('dash')">Labeled Bot</a><a onclick="show('branding')">Logo & Banners</a><a onclick="show('blog')">Blogs</a><a onclick="show('video')">Videos</a><a onclick="show('inject')">Injectors</a><a onclick="show('security')">🛡️ Security</a><a href="/">Site</a></div>
    <div class="main">
        <div id="dash" class="panel active"><h3>Bot Control (24 Modules)</h3>
            <div class="cmd-grid">${CMD_LIST.map((n, i)=>`<button class="cmd-btn" onclick="run(${i+1})">${i+1}. ${n}</button>`).join('')}</div>
        </div>
        ...
        <div id="security" class="panel"><h3>🛡️ Credentials</h3><form action="/admin/change-password" method="POST"><label>New Username</label><input name="newUser" value="${data.adminAuth.user}"><label>New Password</label><input type="password" name="newPassword" required><button>Lock Password</button></form></div>
    </div>
    <script>
...
app.get('/library', (req, res) => { res.send("<h1>Library Gated. Login required.</h1><a href='/'>Back</a>"); });
...
app.listen(PORT, '0.0.0.0', () => console.log(`🚀 EMPIRE MASTER READY ON PORT ${PORT}`));
